...
const avatarFallback = computed(() => {
  const text = auth.user?.nickname || auth.user?.username || "用户"
...
onMounted(() => {
  const cachedCity = localStorage.getItem("yutu-current-city")
  const cacheNeedsRefresh =
    !cachedCity ||
    cachedCity.includes("镇") ||
    cachedCity.includes("街道") ||
    cachedCity.includes(" ") ||
    ((cachedCity.endsWith("区") || cachedCity.endsWith("县")) &&
      !cachedCity.includes("省") &&
      !cachedCity.includes("市") &&
      !cachedCity.includes("自治区"))
  if (
    cacheNeedsRefresh
  ) {
    locateCurrentCity()
