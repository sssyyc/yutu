...
<template>
  <div class="admin-module-page">
          ...
          <div class="departure-manager-shell">
            <div class="departure-toolbar">
              ...
              <div class="departure-toolbar-actions">
                ...
                <el-button type="primary" plain :disabled="!list.length" @click="openAddDepartureDialog">
                  新增日期
                </el-button>
                <el-button
                  type="primary"
                ...
                <el-table-column label="操作" width="110" fixed="right">
                  <template #default="{ row }">
                    <el-button
                      ...
                      text
                      type="danger"
                      @click="removeDepartureDate(row.key)"
                    >
                      下架
                    </el-button>
                    <el-button
    ...
    <el-dialog
      v-model="addDepartureDialogVisible"
      width="520px"
      class="departure-create-dialog"
      destroy-on-close
      title="新增出发日期"
    >
      <el-form label-width="88px" class="departure-create-form">
        <el-form-item label="选择路线">
          <el-select
            v-model="addDepartureForm.routeId"
            filterable
            class="departure-create-field"
            placeholder="请选择路线"
            @change="handleAddDepartureRouteChange"
          >
            <el-option
              v-for="item in sortedManagedRoutes"
              :key="item.id"
        ...
        <el-form-item label="出发日期">
          <el-date-picker
            v-model="addDepartureForm.departDate"
            type="date"
            format="YYYY-MM-DD"
            value-format="YYYY-MM-DD"
            placeholder="选择出发日期"
        ...
        <el-form-item label="售价">
          <el-input-number
            v-model="addDepartureForm.salePrice"
            :min="0"
            :precision="2"
            controls-position="right"
            class="departure-create-field"
        ...
        <el-form-item label="余量">
          <el-input-number
            v-model="addDepartureForm.remainCount"
            :min="0"
            controls-position="right"
            class="departure-create-field"
          />
      ...
      <div class="departure-create-tip">确认后会加入当前编辑列表，点击“保存出发日期”后才会正式提交审核。</div>

      <template #footer>
        <el-button @click="addDepartureDialogVisible = false">取消</el-button>
        <el-button type="primary" @click="confirmAddDepartureDate">加入列表</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
const addDepartureDialogVisible = ref(false);
...
const routeExpiryMap = computed(() => {
  ...
  allDepartureDates.value.forEach((item) => {
    ...
    if (!groupedDates.has(routeId)) groupedDates.set(routeId, []);
    if (item.departDate) groupedDates.get(routeId).push(String(item.departDate));
  });
...
const canEditDepartureDates = computed(() => Boolean(selectedManagedRoute.value));

let departureDateKeySeed = 0;
const departureDates = ref([createDepartureDateItem()]);
const addDepartureForm = ref(createAddDepartureForm());
...
function createAddDepartureForm(route = null) {
  const targetRoute = route || selectedManagedRoute.value || sortedManagedRoutes.value[0] || null;
  return {
    routeId: targetRoute?.id ?? null,
    departDate: "",
    salePrice: Number(targetRoute?.price || 0),
    remainCount: Number(targetRoute?.stock || 0)
  };
}

function syncAddDepartureForm(routeId) {
  const targetRoute =
    list.value.find((item) => Number(item.id) === Number(routeId)) ||
    selectedManagedRoute.value ||
    sortedManagedRoutes.value[0] ||
    null;
  addDepartureForm.value = createAddDepartureForm(targetRoute);
}
...
function normalizeDepartureDates(rows = []) {
  if (!rows.length) return [createDepartureDateItem()];
...
function openAddDepartureDialog() {
  if (!list.value.length) {
    ElMessage.warning("请先新增路线");
    return;
  }
  syncAddDepartureForm(selectedManagedRoute.value?.id);
  addDepartureDialogVisible.value = true;
}

function handleAddDepartureRouteChange(routeId) {
  syncAddDepartureForm(routeId);
}

async function confirmAddDepartureDate() {
  const routeId = Number(addDepartureForm.value.routeId);
  const targetRoute = list.value.find((item) => Number(item.id) === routeId);
  const payload = {
    departDate: String(addDepartureForm.value.departDate || "").trim(),
    salePrice: Number(addDepartureForm.value.salePrice),
    remainCount: Number(addDepartureForm.value.remainCount)
  };
  ...
  if (Number(managedRouteId.value) !== routeId) {
    managedRouteId.value = routeId;
    await loadManagedRouteDates();
  }

  departureDates.value.push(
    createDepartureDateItem({
      routeId,
      routeName: targetRoute.routeName,
      routeAuditStatus: Number(targetRoute.auditStatus ?? 0),
      routePublishStatus: Number(targetRoute.publishStatus ?? 0),
  ...
  addDepartureDialogVisible.value = false;
  ElMessage.success("已加入列表，请继续点击“保存出发日期”完成提交");
}
...
async function loadDepartureTabData(force = false) {
  if (departureDataLoaded.value && !force) return;
...
async function load() {
  await loadRouteList();
  if (activeTab.value === "departure") await loadDepartureTabData(true);
}
...
async function saveDepartureDates() {
  ...
  if (!validateDepartureDates(payload)) return;
...
async function deleteOverviewDepartureDate(row) {
  ...
  try {
    ...
    await loadAllDepartureDates();
    departureDataLoaded.value = true;
    if (Number(managedRouteId.value) === routeId) await loadManagedRouteDates();
...
watch(activeTab, async (value) => {
  if (value === "departure") await loadDepartureTabData();
});
...
watch(
  ...
  (routes) => {
    if (!routes.length) {
      addDepartureDialogVisible.value = false;
      addDepartureForm.value = createAddDepartureForm();
      return;
    }
    if (!routes.some((item) => Number(item.id) === Number(addDepartureForm.value.routeId))) {
      addDepartureForm.value = createAddDepartureForm(routes[0]);
    }
  },
  { immediate: true }
);
...
.departure-create-form {
  padding-top: 4px;
}

:deep(.route-detail-dialog .el-dialog),
:deep(.departure-create-dialog .el-dialog) {
  border-radius: 18px;
  overflow: hidden;
}
