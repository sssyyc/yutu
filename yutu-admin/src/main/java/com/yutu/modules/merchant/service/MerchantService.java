...
public class MerchantService {
    ...
    public Long createRoute(MerchantRouteSaveRequest request) {
        ...
        applyContractTemplates(route, request);
    ...
    public List<ContractTemplate> availableContractTemplates(Long categoryId) {
        TourCategory category = tourCategoryMapper.selectById(categoryId);
    ...
    private boolean hasRouteBaseChanged(TourRoute route, MerchantRouteSaveRequest request) {
        return !Objects.equals(route.getCategoryId(), request.getCategoryId())
                || !Objects.equals(route.getStandardTemplateId(), request.getStandardTemplateId())
                || !Objects.equals(route.getRouteTemplateId(), request.getRouteTemplateId())
                || !Objects.equals(route.getSupplementTemplateId(), request.getSupplementTemplateId())
    ...
    private void applyContractTemplates(TourRoute route, MerchantRouteSaveRequest request) {
        Long categoryId = request.getCategoryId();
        TourCategory category = categoryId == null ? null : tourCategoryMapper.selectById(categoryId);
        String categoryName = category == null ? null : category.getCategoryName();

        ContractTemplate standardTemplate = validateTemplate(request.getStandardTemplateId(), "STANDARD", categoryName, true);
        ContractTemplate routeTemplate = validateTemplate(request.getRouteTemplateId(), "ROUTE", categoryName, true);
        ContractTemplate supplementTemplate = validateTemplate(request.getSupplementTemplateId(), "SUPPLEMENT", categoryName, false);

        route.setStandardTemplateId(standardTemplate == null ? null : standardTemplate.getId());
        route.setRouteTemplateId(routeTemplate == null ? null : routeTemplate.getId());
        route.setSupplementTemplateId(supplementTemplate == null ? null : supplementTemplate.getId());
    }

    private ContractTemplate validateTemplate(Long templateId, String templateType, String applyScope, boolean required) {
        if (templateId == null) {
            if (required) {
                throw new BizException(400, "请选择合同模板");
            }
            return null;
        }
        ContractTemplate template = contractTemplateMapper.selectById(templateId);
        if (template == null || !Objects.equals(template.getStatus(), 1)) {
            throw new BizException(400, "所选合同模板不存在或未启用");
        }
        if (!Objects.equals(template.getTemplateType(), templateType)) {
            throw new BizException(400, "所选合同模板类型不匹配");
        }
        if (StringUtils.hasText(applyScope) && !Objects.equals(template.getApplyScope(), applyScope)) {
            throw new BizException(400, "所选合同模板适用范围与路线形式不一致");
        }
        return template;
    }
