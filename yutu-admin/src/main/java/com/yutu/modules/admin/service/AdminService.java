...
import com.yutu.modules.admin.vo.AdminMerchantRecordVO;
...
public class AdminService {
    ...
    public List<AdminMerchantVO> merchants(String keyword) {
        ...
        Map<Long, Long> routeCountMap = routes.stream()
                .filter(item -> item.getMerchantId() != null)
                .collect(Collectors.groupingBy(TourRoute::getMerchantId, Collectors.counting()));
        Map<Long, Long> orderCountMap = orders.stream()
                .filter(item -> item.getMerchantId() != null)
                .collect(Collectors.groupingBy(TourOrder::getMerchantId, Collectors.counting()));
        Map<Long, BigDecimal> turnoverMap = orders.stream()
                .filter(item -> item.getMerchantId() != null)
                .filter(item -> "PAID".equals(item.getPayStatus()) || "REFUNDED".equals(item.getPayStatus()))
                .collect(Collectors.groupingBy(
                        TourOrder::getMerchantId,
                        Collectors.reducing(
                                BigDecimal.ZERO,
                                item -> item.getPayAmount() == null ? BigDecimal.ZERO : item.getPayAmount(),
                                BigDecimal::add
                        )));
        Map<Long, Long> complaintCountMap = complaints.stream()
                .filter(item -> item.getMerchantId() != null)
        ...
        List<AdminMerchantVO> result = merchants.stream().map(shop -> {
            ...
            vo.setRouteCount(routeCountMap.getOrDefault(shop.getId(), 0L));
            vo.setOrderCount(orderCountMap.getOrDefault(shop.getId(), 0L));
            vo.setTurnover(turnoverMap.getOrDefault(shop.getId(), BigDecimal.ZERO));
            vo.setComplaintCount(complaintCountMap.getOrDefault(shop.getId(), 0L));
            vo.setWarned(isWarned(shop));
            return vo;
        }).collect(Collectors.toList());
    ...
    public AdminMerchantRecordVO merchantRecords(Long id) {
        ...
        List<TourRoute> routes = tourRouteMapper.selectList(new LambdaQueryWrapper<TourRoute>()
                .eq(TourRoute::getMerchantId, id)
                .orderByDesc(TourRoute::getUpdateTime));
        List<TourOrder> orders = tourOrderMapper.selectList(new LambdaQueryWrapper<TourOrder>()
                .eq(TourOrder::getMerchantId, id)
                .orderByDesc(TourOrder::getCreateTime));
        List<ComplaintOrder> complaints = complaintOrderMapper.selectList(new LambdaQueryWrapper<ComplaintOrder>()
                .eq(ComplaintOrder::getMerchantId, id)
        ...
        List<RefundOrder> refunds = refundOrderMapper.selectList(new LambdaQueryWrapper<RefundOrder>()
                .eq(RefundOrder::getMerchantId, id)
                .orderByDesc(RefundOrder::getCreateTime));

        AdminMerchantRecordVO vo = new AdminMerchantRecordVO();
        vo.setMerchantId(shop.getId());
        vo.setShopName(shop.getShopName());
        vo.setStatus(shop.getStatus());
        vo.setAuditStatus(shop.getAuditStatus());
        vo.setLatestRemark(shop.getAuditRemark());
        vo.setRouteCount(routes.size());
        vo.setPublishedRouteCount(routes.stream().filter(item -> Objects.equals(item.getPublishStatus(), 1)).count());
        vo.setOrderCount(orders.size());
        vo.setPaidOrderCount(orders.stream().filter(item -> "PAID".equals(item.getPayStatus()) || "REFUNDED".equals(item.getPayStatus())).count());
        vo.setTurnover(orders.stream()
                .filter(item -> "PAID".equals(item.getPayStatus()) || "REFUNDED".equals(item.getPayStatus()))
                .map(item -> item.getPayAmount() == null ? BigDecimal.ZERO : item.getPayAmount())
                .reduce(BigDecimal.ZERO, BigDecimal::add));
        vo.setComplaintCount(complaints.size());
        vo.setRefundCount(refunds.size());
        vo.setRecentRoutes(routes.stream().limit(5).map(item -> {
            AdminMerchantRecordVO.RouteItem routeItem = new AdminMerchantRecordVO.RouteItem();
            routeItem.setId(item.getId());
            routeItem.setRouteName(item.getRouteName());
            routeItem.setAuditStatus(item.getAuditStatus());
            routeItem.setPublishStatus(item.getPublishStatus());
            routeItem.setUpdateTime(item.getUpdateTime());
            return routeItem;
        }).collect(Collectors.toList()));
        vo.setRecentOrders(orders.stream().limit(5).map(item -> {
            AdminMerchantRecordVO.OrderItem orderItem = new AdminMerchantRecordVO.OrderItem();
            orderItem.setId(item.getId());
            orderItem.setOrderNo(item.getOrderNo());
            orderItem.setOrderStatus(item.getOrderStatus());
            orderItem.setPayStatus(item.getPayStatus());
            orderItem.setPayAmount(item.getPayAmount());
            orderItem.setCreateTime(item.getCreateTime());
            return orderItem;
        }).collect(Collectors.toList()));
        vo.setRecentComplaints(complaints.stream().limit(5).map(item -> {
            AdminMerchantRecordVO.ComplaintItem complaintItem = new AdminMerchantRecordVO.ComplaintItem();
            complaintItem.setId(item.getId());
            complaintItem.setComplaintNo(item.getComplaintNo());
            complaintItem.setComplaintType(item.getComplaintType());
            complaintItem.setStatus(item.getStatus());
            complaintItem.setTitle(item.getTitle());
            complaintItem.setCreateTime(item.getCreateTime());
    ...
    @Transactional(rollbackFor = Exception.class)
    public void merchantWarn(Long id, MerchantAuditRequest request) {
        MerchantShop merchantShop = merchantShopMapper.selectById(id);
        if (merchantShop == null) {
            throw new BizException(404, "商户不存在");
        }
        String reason = request == null ? "" : defaultIfBlank(request.getAuditRemark(), "");
        if (!StringUtils.hasText(reason)) {
            throw new BizException(400, "请填写预警说明");
        }
        merchantShop.setAuditRemark("【预警】" + reason.trim());
        merchantShop.setAuditTime(LocalDateTime.now());
        merchantShopMapper.updateById(merchantShop);
    }

    @Transactional(rollbackFor = Exception.class)
    public void merchantBan(Long id, boolean ban, MerchantAuditRequest request) {
        MerchantShop merchantShop = merchantShopMapper.selectById(id);
        if (merchantShop == null) {
            throw new BizException(404, "商户不存在");
        }
        merchantShop.setStatus(ban ? 0 : 1);
        String reason = request == null ? "" : defaultIfBlank(request.getAuditRemark(), "");
        if (ban && StringUtils.hasText(reason)) {
            merchantShop.setAuditRemark("【封禁】" + reason.trim());
        } else if (!ban && StringUtils.hasText(reason)) {
            merchantShop.setAuditRemark("【解除封禁】" + reason.trim());
        }
        merchantShop.setAuditTime(LocalDateTime.now());
        merchantShopMapper.updateById(merchantShop);

        SysUser user = merchantShop.getUserId() == null ? null : sysUserMapper.selectById(merchantShop.getUserId());
        if (user != null) {
    ...
    private boolean isWarned(MerchantShop shop) {
        return shop != null
                && StringUtils.hasText(shop.getAuditRemark())
                && shop.getAuditRemark().trim().startsWith("【预警】");
    }
